...
dict_lidar = pd.read_pickle("C:/Users/tgilh/hublidardatabase/files/df_power_law.pkl")
df_v_y_extracted=dict_lidar['dtu_10mw_wsp_11.4_seed_927_ae_0.181_power_law']['config_1']
...
xlist_for_plot  = df_v_y_extracted[df_v_y_extracted['Beam_No'] == 'b1']['Time']
ylist_for_plot  = df_v_y_extracted[df_v_y_extracted['Beam_No'] == 'b1']['HuLi_V_LOS_wgh_scaled']
...
# df_v_y_extracted = df_v_y_extracted.drop(['shaft_rot_angle', 'hub1_pos_x', 'hub1_pos_y', 'hub1_pos_z',
#                                           'HuLi_Yt', 'x_user', 'y_user', 'z_user', 'xt_user',], axis=1)

df_v_y_extracted = df_v_y_extracted.drop(['shaft_rot_angle', 'hub1_pos_x', 'hub1_pos_y', 'hub1_pos_z',
        'HuLi_Yt', 'x_user', 'y_user',
       'z_user', 'xt_user', 'time_tbox', 'HuLi_V_LOS_nom',
       'HuLi_V_LOS_nom_scaled', 'Theta', 'Psi'], axis=1)
...
def getSpeedAtZ_ref():
    ...
    return lowerHeightSpeed,lowerHeight,upperHeightSpeed,upperHeight
...
def plot_vertical_true_power(df_x,df_y, marker):
    ...
    plt.figure()    
    plt.scatter(trueSpeeds,     trueHeights, color = 'blue', label = 'True Power Law', s =1)
    plt.scatter(df_x, df_y, color='red', label='binned speeds', s = 5)    
    plt.plot([], [], ' ', label=f'Alpha ("True")={trueAlpha:.2f}')    
    plt.plot([], [], ' ', label=f'Advection Speed = {u_adv_true:.2f}')    
    plt.plot([], [], ' ', label=f'Alpha (fitted) = {alpha_curved:.2f}')    
...
def print_value(name,type_data,df):
    ...
    if len(non_nan_values) > 0:
        print(f'Mean {type_data} ({name} fit) = {np.mean(non_nan_values):.2}')
    else:
...
D = 178
###########BIN SIZE#################
bin_size = 10 # Height bin size in meters
###########TIME SCALE###############
timescale = 400 # Time scales in seconds
...
u_adv_initial = 11.4
u_adv_true_speeds = [7.98,10.53,13.06,15.54]
u_adv_true_speeds = [11.4,11.4,11.4,11.4,11.4,11.4,11.4,11.4]
...
t_fill_up = 121 #after 150 seconds the array is full and the algorith is as accurate as it can be
buffer = 0
...
alpha_changing_times = [446,1346,2246,3146,4046,4096,t_stop]

alpha_variable = 0
alpha_time = alpha_changing_times[alpha_variable]
    

# Use np.arange to create the time array with a step size of dt_samp
time_arr = np.arange(t_start, t_stop + dt_average, dt_samp)  # Added t_stop + dt_samp to include t_stop
...
x_range = [-200,200]
y_range = [45,500]
z_range = [0,400] #box height
v_range = [df_active['v'].min(),df_active['v'].max()]
#
...
for t in time_arr:
    ...
    if not df_grouped['z_avg_numeric'].empty and not df_grouped['u'].empty:
        ...
        if t > t_fill_up+t_start and t < t_cut_off:
            #perform fitting at a lower frequency than binning (dt_average)
            if t-t_last_average_time >= dt_average:
                # print('############## ESTIMATION ##############')
                # print(f"Averaging binned values every {dt_average}s, with a window of {timescale}s and fitting at t={t}")
                ...
                lowerHeightSpeed,lowerHeight,upperHeightSpeed,upperHeight = getSpeedAtZ_ref()
                # v_ref_binned = linInterp(lowerHeight, upperHeight, lowerHeightSpeed, upperHeightSpeed, z_ref)
                # print(f'v_ref_binned = {v_ref_binned:.2f} [u(z_ref)]' )               
                # =============================================================================
                # PLOTS:    VERTICAL WIND PROFILE
                # =============================================================================
                # plot_vertical_specific(averaged_df['u'], averaged_df['z_avg_numeric'], plot_vertical_m) # plot the vertical wind profile with storage_df (average values for each bin size over dt_averaged seconds only)    
                # plot_vertical_true_power(averaged_df['u'], averaged_df['z_avg_numeric'], plot_vertical_w_true_power) # plot the vertical wind profile with storage_df (average values for each bin size over dt_averaged seconds only)    
                ...
                if t>=alpha_changing_times[alpha_variable]:
                    alpha_variable +=1
                    trueAlpha=trueAlphas[alpha_variable]
                    u_adv_true=u_adv_true_speeds[alpha_variable]
...
# plot_parameters_vs_time([0.1,0.4])
# plot_amount_of_points_vs_time()
# plot_mean_value_vs_time(rmse_df,'RMSE',[0,2],legend=True)
# plot_mean_value_vs_time(advection_df,'u_adv_mean',[6,10],u_adv_true, mean=True, legend=True)
times = np.linspace(df_active['Time'].min(),df_active['Time'].max(),len(params_df))
alpha_changing_times_plot = [0,1346,1346,2246,2246,3146,3146,4046]
...
plt.plot(times,params_df['alpha_curved'],label = 'alpha_curved')
# plt.plot(times,params_df['average_alpha'], label = 'average_alpha')
plt.plot(alpha_changing_times_plot,true_alphas_plot)
plt.ylim(0,0.3)
plt.legend()
plt.show()

times = np.linspace(df_active['Time'].min(),df_active['Time'].max(),len(advection_df))
advec_changing_times_plot = [0,80,80,940,940,1840,1840,2740,2740,3700]
# true_advec_plot = [4.56,4.56,7.98,7.98,10.53,10.53,13.06,13.06,15.54,15.54]
true_advec_plot = [11.4]*len(advec_changing_times_plot)
advection_df['average_advec'] = advection_df[['Log', 'Poly']].mean(axis=1)

plt.figure()
# plt.plot(times,advection_df['Power_Curved'],label='alpha_linear')
# plt.plot(times,advection_df['Power_Linear'],label = 'Power_Linear')
# plt.plot(times,advection_df['Log'], label = 'Log')
plt.plot(times,advection_df['average_advec'], label = 'average_advec')
plt.plot(advec_changing_times_plot,true_advec_plot,label='True Mean Advection Speed',linestyle='--')
...
times = np.linspace(df_active['Time'].min(), df_active['Time'].max(), len(params_df))
alpha_changing_times_plot = [0, 1183, 1184, 1974, 1975, 2765, 2766, 3700]
alpha_changing_times_plottimescale = [0,timescale,timescale,timescale*2,timescale*2,timescale*3,timescale*3,timescale*4,timescale*4,timescale*5,timescale*5,timescale*6,timescale*6,timescale*7,timescale*7,timescale*8,timescale*8,timescale*9,timescale*9,timescale*10,timescale*10,timescale*11,timescale*11,timescale*12,timescale*12]

# Convert alpha_changing_times_plot to pairs of intervals
alpha_intervals = [(alpha_changing_times_plot[i], alpha_changing_times_plot[i+1]) 
                   for i in range(len(alpha_changing_times_plot) - 1)]

alpha_intervals_timescale = [(alpha_changing_times_plottimescale[i], alpha_changing_times_plottimescale[i+1]) 
                   for i in range(len(alpha_changing_times_plottimescale) - 1)]

# Compute the average alphas for each interval (original)
average_alphas = []
for start, end in alpha_intervals:
    mask = (times >= start) & (times <= end)
    avg_alpha = params_df.loc[mask, 'alpha_curved'].mean()
    average_alphas.append(avg_alpha)

# Compute the average alphas for each interval (timescale-based)
average_alphastimescale = []
for start, end in alpha_intervals_timescale:
    mask = (times >= start) & (times <= end)
    avg_alpha = params_df.loc[mask, 'alpha_curved'].mean()
    average_alphastimescale.append(avg_alpha)

# Create DataFrame for original intervals
average_alphas_df = pd.DataFrame({
    'Interval_Start': [start for start, _ in alpha_intervals],
    'Interval_End': [end for _, end in alpha_intervals],
    'Average_Alpha': average_alphas
})

# Create DataFrame for timescale intervals
average_alphas_dftimescale = pd.DataFrame({
    'Interval_Start': [start for start, _ in alpha_intervals_timescale],
    'Interval_End': [end for _, end in alpha_intervals_timescale],
    'Average_Alpha': average_alphastimescale
})

# Forward-fill the NaNs (if any)
average_alphas_df['Average_Alpha'] = average_alphas_df['Average_Alpha'].ffill()
average_alphas_dftimescale['Average_Alpha'] = average_alphas_dftimescale['Average_Alpha'].ffill()

# Add the last row based on the final Interval End and the last Average_Alpha value for each DataFrame
last_interval_start = average_alphas_df['Interval_End'].iloc[-1]
last_average_alpha = average_alphas_df['Average_Alpha'].iloc[-1]

last_interval_starttimescale = average_alphas_dftimescale['Interval_End'].iloc[-1]
last_average_alphatimescale = average_alphas_dftimescale['Average_Alpha'].iloc[-1]

# Create the last row to append (original)
last_row = pd.DataFrame({
    'Interval_Start': [last_interval_start],
    'Interval_End': [last_interval_start],  # Interval end should be the same as the start
    'Average_Alpha': [last_average_alpha]
})

# Create the last row to append (timescale-based)
last_rowtimescale = pd.DataFrame({
    'Interval_Start': [last_interval_starttimescale],
    'Interval_End': [last_interval_starttimescale],  # Interval end should be the same as the start
    'Average_Alpha': [last_average_alphatimescale]
})

# Append the last row to the DataFrame
average_alphas_df = pd.concat([average_alphas_df, last_row], ignore_index=True)
average_alphas_dftimescale = pd.concat([average_alphas_dftimescale, last_rowtimescale], ignore_index=True)

true_alphas_plot = [0.2,0.2,0.15,0.15,0.125,0.125,0.1,0.1]
# true_alphas_plot = [0.2]*(len(alpha_changing_times)+1)
params_df['average_alpha'] = params_df[['alpha_linear', 'alpha_curved']].mean(axis=1)
plt.figure()
# plt.plot(times,params_df['alpha_linear'],label='alpha_linear')
plt.plot(times, params_df['alpha_curved'], label='Estimated alpha (f=1/30s)',color='black',alpha=0.3)
# plt.plot(average_alphas_df['Interval_Start'], average_alphas_df['Average_Alpha'],
#           label='Estimated alpha in steps = full period',linewidth=3,color='blue',linestyle = ':')
plt.plot(average_alphas_dftimescale['Interval_Start'], average_alphas_dftimescale['Average_Alpha'],
          label=f'Estimated alpha in steps = {timescale} s',linewidth=3,color='black', linestyle='-')
# plt.plot(times,params_df['average_alpha'], label = 'average_alpha',alpha=0.1)
plt.plot(alpha_changing_times_plot[0:2], true_alphas_plot[0:2], label='True alpha',
         linestyle='--',color='red',linewidth=3)
plt.plot(alpha_changing_times_plot[1:3], true_alphas_plot[1:3], 
         linestyle='--',color='red',linewidth=3)
plt.plot(alpha_changing_times_plot[2:4], true_alphas_plot[2:4], 
         linestyle='--',color='blue',linewidth=3)
plt.plot(alpha_changing_times_plot[3:5], true_alphas_plot[3:5], 
         linestyle='--',color='blue',linewidth=3)
plt.plot(alpha_changing_times_plot[4:6], true_alphas_plot[4:6], 
         linestyle='--',color='brown',linewidth=3)
plt.plot(alpha_changing_times_plot[5:7], true_alphas_plot[5:7], 
         linestyle='--',color='brown',linewidth=3)
plt.plot([2766,3700], [0.1,0.1], 
         linestyle='--',color='deepskyblue',linewidth=3)
# plt.plot(alpha_changing_times_plot[0,1], true_alphas_plot[0,1], label='True alpha',
#          linestyle='--',color='red',linewidth=3)
# plt.plot([0,3700],[params_df['alpha_curved'].mean(),params_df['alpha_curved'].mean()],
#          label=f"estimated global mean = {params_df['alpha_curved'].mean():.3f} ",linestyle='-.',
#          color='green',linewidth=2)

plt.title('Alpha Estimation: Time-Varying Alpha')
plt.ylim(0.08,0.25)
plt.xlim(-30,3700)
plt.xlabel('time [s]')
plt.ylabel('alpha [-]')
...
plt.scatter(df_timelist_plot['Time']-25, df_timelist_plot['v'], color='black',  s = .5,label='u: 110m-130m',alpha=0.5)
plt.plot(times,advection_df['average_advec'], label = 'U Estimation',linewidth=6, color='black')
# plt.plot(advec_changing_times_plot,true_advec_plot,label='"True U"',linestyle='--',linewidth=3,color='blue')
# plt.plot([], [], ' ', label=f"Average = {np.mean(advection_df['average_advec']):.3f}")    
# plt.plot([], [], ' ', label=f"True mean advection speed = {np.mean(true_advec_plot):.3f}")
plt.plot(alpha_changing_times_plot[0:2], [11.4,11.4], label='True U',
         linestyle='--',color='red',linewidth=5)
plt.plot(alpha_changing_times_plot[1:3], [11.4,11.4], 
         linestyle='--',color='red',linewidth=5)
plt.plot(alpha_changing_times_plot[2:4],[11.4,11.4], 
         linestyle='--',color='blue',linewidth=5)
plt.plot(alpha_changing_times_plot[3:5], [11.4,11.4], 
         linestyle='--',color='blue',linewidth=5)
plt.plot(alpha_changing_times_plot[4:6],[11.4,11.4],  
         linestyle='--',color='red',linewidth=5)
plt.plot(alpha_changing_times_plot[5:7], [11.4,11.4], 
         linestyle='--',color='red',linewidth=5)
plt.plot([2766,3700],[11.4,11.4],  
         linestyle='--',color='blue',linewidth=5)    
plt.plot([0,3700],[advection_df['average_advec'].mean(),advection_df['average_advec'].mean()],label=f'global estimated mean U = {advection_df["average_advec"].mean():.2f} m/s')
plt.ylim(7,18)
plt.xlabel('Time [s]')
plt.ylabel('Wind Speed [m/s]')
plt.xlim(0,df_timelist_plot['Time'].max())
plt.title('U Estimation: Time-Varying Alpha')
plt.legend(loc='upper center')
plt.show()

#%%
#%% Advection speed
#%% Corrected Advection Speed with Proper Time Alignment

# Create advection intervals from the changing times
advec_intervals = [(advec_changing_times_plot[i], advec_changing_times_plot[i+1]) 
                   for i in range(len(advec_changing_times_plot) - 1)]

# Calculate the average advection speed for each interval
average_advec = []
for start, end in advec_intervals:
    mask = (times >= start) & (times <= end)
    avg_advec = advection_df.loc[mask, 'average_advec'].mean()
    average_advec.append(avg_advec)

# Create a DataFrame for the intervals and average advection speeds
average_advec_df = pd.DataFrame({
    'Interval_Start': [start for start, _ in advec_intervals],
    'Interval_End': [end for _, end in advec_intervals],
    'Average_Advec': average_advec
})

# Now repeat each average value for each step in time
repeated_average_advec = []

# Repeat each value for both start and end of the interval
for i in range(len(average_advec_df)):
    avg_advec = average_advec_df['Average_Advec'].iloc[i]
    start_time = average_advec_df['Interval_Start'].iloc[i]
    end_time = average_advec_df['Interval_End'].iloc[i]

    # Repeat the average advection value for both start and end
    repeated_average_advec.append([start_time, start_time, avg_advec])
    repeated_average_advec.append([end_time, end_time, avg_advec])

# Flatten the repeated list and create a new DataFrame
repeated_df = pd.DataFrame(repeated_average_advec, columns=['Interval_Start', 'Interval_End', 'Average_Advec'])

# Forward-fill NaNs if any exist in 'Average_Advec' (if any values are missing)
repeated_df['Average_Advec'] = repeated_df['Average_Advec'].ffill()

# Handle the last row based on the final interval
last_interval_start = repeated_df['Interval_End'].iloc[-1]
last_average_advec = repeated_df['Average_Advec'].iloc[-1]

# Add the last row to continue the last interval
last_row_advec = pd.DataFrame({
    'Interval_Start': [last_interval_start],
    'Interval_End': [last_interval_start + 1],  # Extend the interval by 1
    'Average_Advec': [last_average_advec]
})

# Append the last row to the DataFrame
repeated_df = pd.concat([repeated_df, last_row_advec], ignore_index=True)


# Plotting the result
plt.figure()
plt.scatter(df_timelist_plot['Time']-25, df_timelist_plot['v'], color='grey', s=0.5, label='u: 110m-130m')
plt.plot(times, advection_df['average_advec'], label='U Estimation', linewidth=4, color='black')
plt.plot(advec_changing_times_plot, true_advec_plot, label='"True U"', 
         linestyle='--', linewidth=5, color='red')
plt.plot(repeated_df['Interval_Start'], repeated_df['Average_Advec'],
         label='Estimated U in steps',linewidth=5,color='blue',linestyle = ':')
# plt.ylim(df_active['v'].min(), df_active['v'].max())
plt.ylim(5,21)
plt.xlabel('Time [s]')
plt.ylabel('Wind Speed [m/s]')
plt.xlim(0, df_timelist_plot['Time'].max())
plt.title('Time Response Step Function Wind')
plt.legend()
plt.show()
