...
def truePowerLaw(trueAlpha, trueV_ref,z_ref):
...
def plot_vertical_true_power(df_x,df_y, marker):
    ...
    trueSpeeds, trueHeights = truePowerLaw(trueAlpha, V_ref_true, z_ref)
...
def plot_mean_value_vs_time(df,name,ylim, advec = False, mean = False, legend= False, legendPos = 'lower right'):
    ...
    ExpectedValList = [expectedMeanAdvecWs] * len(time_values)
    ...
    plt.plot(time_values, ExpectedValList, label=f'expected u_adv={expectedMeanAdvecWs:.1f}',color='black')  
    # if advec == True:
...
def print_rmse():
    ...
    print('RMSE Full Height (Best fit)')
...
def print_advection():    
    print('Best/Last Advection speeds from each fit:')
...
def print_best_vals():
    """
    Prints the 'best' values for all the fits: Power_Curved, Power_Linear, Log, and Poly.
    """
    print()
    print_rmse()
    print()
...
def print_all_means():
    ...
    """
    print()
    print('Last/Best values for post processing and evaluation')
    print_rmse()
    print()
    print_advection()
    print()
...
###########TIME SCALE###############
timescale = 600 # Time scales in seconds
...
expectedMeanAdvecWs = u_adv_true
trueAlpha = 0.2
alpha_initial_guess = 0.12
...
time_arr = np.arange(t_start, t_stop + dt_samp, dt_samp)  # Added t_stop + dt_samp to include t_stop
    ...
    if not df_grouped['z_avg_numeric'].empty and not df_grouped['u'].empty:
        ...
        if t > t_fill_up+t_start and t < t_cut_off:
            ...
            if t-t_last_average_time >= dt_average:
                ...
                plot_vertical_true_power(averaged_df['u'], averaged_df['z_avg_numeric'], plot_vertical_true_power) # plot the vertical wind profile with storage_df (average values for each bin size over dt_averaged seconds only)    
                ...
                #             POWER CURVED FIT 
                # =============================================================================            
                power_curved_fitted_speeds, rmse_rotor_plane_curved, rmse_full_height_curved, rmse_binned_speeds_curved, alpha_curved = tryFitPower(power_curved_fit,alpha_initial_guess,V_ref_initial) #DO THE FIT
...
# plot_mean_value_vs_time(advection_df,'u_adv_mean',[6,10],expectedMeanAdvecWs, mean=True, legend=True)
