...
def plot_vertical_true_power(df_x,df_y, marker=False):
    """
    Plots vertical wind profile with two sets of wind speeds and heights.
    Optional: includes RMSE and additional values like counts or points if provided.
    """
    ...
    plt.plot([], [], ' ', label=f'True Alpha={trueAlpha:.2f}')    
    plt.plot([], [], ' ', label=f'Advection Speed = {u_adv_true:.2f}')    
    plt.plot([], [], ' ', label=f'filtered V_ref= {v_ref_binned:.2f}')    
    plt.plot([], [], ' ', label=f'filtered avg advec= {np.mean(df_x):.2f}')
...
###########TIME SCALE###############
timescale = 420 # Time scales in seconds
...
plot_vertical_w_true_power        = False
plot_y_vs_z                       = False
plot_x_vs_y                       = False
plot_x_vs_z                       = False
plot_no_bins                      = False
...
time_arr = np.arange(t_start, t_stop + T_average, dt_samp)  # Added t_stop + dt_samp to include t_stop
...
alpha_changing_times_plot420 = [0,420,420,420*2,420*2,420*3,420*3,420*4,420*4,420*5,420*5,420*6,420*6,420*7,420*7,420*8,420*8,420*9,420*9]
...
alpha_intervals_420 = [(alpha_changing_times_plot420[i], alpha_changing_times_plot420[i+1]) 
                   for i in range(len(alpha_changing_times_plot420) - 1)]
...
# Compute the average alphas for each interval (420-based)
average_alphas420 = []
for start, end in alpha_intervals_420:
    mask = (times >= start) & (times <= end)
    avg_alpha = params_df.loc[mask, 'alpha_curved'].mean()
    average_alphas420.append(avg_alpha)
...
# Create DataFrame for 420 intervals
average_alphas_df420 = pd.DataFrame({
    'Interval_Start': [start for start, _ in alpha_intervals_420],
    'Interval_End': [end for _, end in alpha_intervals_420],
    'Average_Alpha': average_alphas420
})

# Forward-fill the NaNs (if any)
average_alphas_df['Average_Alpha'] = average_alphas_df['Average_Alpha'].ffill()
average_alphas_df420['Average_Alpha'] = average_alphas_df420['Average_Alpha'].ffill()
...
last_interval_start420 = average_alphas_df420['Interval_End'].iloc[-1]
last_average_alpha420 = average_alphas_df420['Average_Alpha'].iloc[-1]
...
# Create the last row to append (420-based)
last_row420 = pd.DataFrame({
    'Interval_Start': [last_interval_start420],
    'Interval_End': [last_interval_start420],  # Interval end should be the same as the start
    'Average_Alpha': [last_average_alpha420]
})

# Append the last row to the DataFrame
average_alphas_df = pd.concat([average_alphas_df, last_row], ignore_index=True)
average_alphas_df420 = pd.concat([average_alphas_df420, last_row420], ignore_index=True)
...
plt.plot(average_alphas_df420['Interval_Start'], average_alphas_df420['Average_Alpha'],
          label='Estimated alpha in steps = 420 s',linewidth=3,color='black', linestyle='-')
...
plt.title('Alpha Estimation: Time-Varying U')
plt.ylim(0.17,0.25)
plt.xlim(0,3600)
plt.xlabel('time [s]')
plt.ylabel('alpha [-]')
...
plt.scatter(df_timelist_plot['Time']-25, df_timelist_plot['v'], color='red',  s = .5,label='u: 110m-130m')
...
plt.ylabel('Wind Speed [m/s]')
plt.xlim(0, df_timelist_plot['Time'].max())
plt.title('U Estimation: Varying U')
plt.legend()
plt.show()
